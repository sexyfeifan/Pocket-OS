...
> v1.9.7 已实施：工作台 `/` 与只读看板 `/view` 分离；加入共享日期/进度规则、查看权限、概览、模板、独立状态、差异预览和持久化排期历史。当前使用与部署说明以 README、CHANGELOG 为准。

## 软件搭建计划书（Development Blueprint）

> v1.9.6 说明：本文件保留早期方案。下文中的倒排、自动前移和全流程重排设计已废弃；当前以 README 的独立手动排期规则为准。
...
> v1.9.6 产品定义：所有节点独立安排。发布日期只约束“发布”工序，Canbox 只填写实际拍摄日期，不推算其他工序。

### 核心痛点

| 痛点 | 现有方案的缺陷 |
|------|---------------|
| 多步骤工作流难以快速建立初始排期，同时自动重排又容易破坏真实安排 | 普通日历缺少工序模板，也无法兼顾已确认的拍摄和剪辑日期 |
...
### Phase 2：排期算法与选题卡逻辑（2-3 天）

**目标**：手动安排为主，需要时主动生成初始排期

- [ ] 实现选题卡四个区域的完整 UI（基础信息、前置准备、制作流程、发布日期）
- [ ] 编写倒排算法（`reverseSchedule`）
- [ ] 实现修改发布日期 → 只同步“发布”工序日期
- [ ] 提供显式“按发布日期倒排”操作，不在普通编辑中自动重排
