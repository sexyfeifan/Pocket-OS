...
function editorStepHtml(t,s,index) {
  ...
  return `<div id="step-row-${index}" data-step-idx="${index}" class="wf-row node-editor ${s.skipped?'node-skipped':''}">
    ...
    <div class="node-actions">${s.skipped?'':`<button class="editor-button primary-action" onclick="toggleStep('${t.id}',${index})">${s.done?'撤回确认':'确认完成'}</button><button class="editor-button" onclick="openNodeException('${t.id}','${s.key}')">延期 / 暂停</button>${s.exception?`<button class="editor-button" onclick="resumeNode('${t.id}','${s.key}')">恢复推进</button>`:''}`}
