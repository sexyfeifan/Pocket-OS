...
(() => {
  ...
  function drawTimeline(root) {
    ...
    const rows=data.single ? data.items[0].productionSteps.filter(s=>S.ranges(s).length).map(s=>({title:s.name,subtitle:W.stateLabels[W.stepState(data.items[0],s)],subtitleHtml:'',events:data.events.filter(e=>!e.note&&e.key===s.key)})) : data.items.map(t=>{
      ...
      const doneText = `${st.done}/${st.total}`;
      const iconsHtml = platformIconsHtml ? `<span class="timeline-icons">${platformIconsHtml}</span>` : '';
      const progressHtml = `<span class="timeline-progress">${doneText}</span>`;
      return {id:t.id,title:t.title,subtitle:'',subtitleHtml:`${iconsHtml}${progressHtml}`,events:data.events.filter(e=>e.topicId===t.id)};
    });
