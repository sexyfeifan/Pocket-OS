...
function renderWorkflowOverview() {
  ...
  const entries=PocketWorkflow.overviewPanels(filteredWorkbenchTopics());
...
function refreshWorkflowStatus(t) {
  ...
  const outlook=document.getElementById('editor-outlook');if(outlook)outlook.textContent=PocketWorkflow.outlookText(t);
  document.querySelectorAll('[data-node-status]').forEach(el=>{const s=t.productionSteps.find(s=>s.key===el.dataset.nodeStatus);if(s){const state=PocketWorkflow.stepState(t,s);el.textContent=PocketWorkflow.stateLabels[state];el.className='node-status status-'+state;}});
...
function duplicateProject(id,title='',progressionMode='calendar'){if(!title){openNewProject('',id);return;}const source=appState.topics.find(t=>t.id===id);if(!source)return;const copy=PocketWorkflow.copyStructure(source);const now=nowISO();const t={id:genId(),title,progressionMode,timeZone:PocketWorkflow.DEFAULT_ZONE,category:copy.category,platforms:copy.platforms,productionSteps:copy.productionSteps,preparationTasks:copy.tasks.map((text,i)=>({id:'prep_'+Date.now()+'_'+i,text,done:false})),publishDate:'',completed:false,lifecycle:progressionMode==='manual'?'preparation':undefined,projectStatus:'normal',createdAt:now,updatedAt:now};appState.topics.push(t);markDirty(t.id);saveData();selectTopic(t.id);logAction('topic.duplicate',t.id,{sourceId:id});}
